var validator = require('../validators/model_validator');
...
*
*/
ActionCache.prototype.getAction = function (action, throwErrorIfNotExist) {

    var name;

    if (typeof(action) === 'object') { 
        if(validator.validateName(action)) {
            name = action.name;
        }
    } else if (typeof(action) === 'string') {
        name = action;
